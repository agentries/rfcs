...
1. **Type code governance**: `typ` identifies the semantic category. Standard Profiles receive one dedicated code per profile in the `0x80-0xEF` range; Private Profiles share `0xF0` and are discriminated by `body.profile` (see §17.2). Fine-grained actions MUST be carried in `body.action`. This prevents type code exhaustion while keeping protocol-level dispatch efficient.
...
- **Standard Profile gate** (`amp-standard-profiles-012-014`): A separate conformance gate for AMP Standard Profiles (RFC 012-014) is defined in `conformance/amp-standard-profiles-012-014-draft-v1/`. Draft gate status: `NOT SATISFIED` (see `conformance/amp-standard-profiles-012-014-draft-v1/GATE-STATUS.md`). Standard Profile vectors are not included in AMP Full gate profiles. Each Standard Profile MUST define its own required test vectors in its respective RFC.
...
4. **Type code agreement**: If both sides declare `typ` for the profile, they MUST agree. If only one side declares `typ`, the other side MUST validate that the declared value matches the registered type code for that profile name (per §17.2). If the declared `typ` does not match the registered mapping, set `matched = false`. If neither declares `typ`, the profile uses `0xF0`. The agreed `typ` is returned in `profile_status`.
